...
**Rule this establishes:** every WebSocket/stream ingestion branch needs a terminal `else` that logs the unrecognised payload. A silently-dropped frame turns a loud vendor error into an invisible outage.
...
scripts/
  ...
  db-diagnose         — why has ingestion stopped? freshness / AIS lock / cursor / write probe
