...
export function buildRegion({ id, bbox, coastElements, waterElements, landSimplifyKm = LAND_SIMPLIFY_KM, outDir = COAST_DIR }) {
  ...
  return emitRegion(id, bbox, land, water, holes, outDir);
}

// Shared emitter — writes frontend/app/coast/<id>.js and returns stats. `holes` is
// just for the stats/comment (the region shape already carries holes in w.h).
function emitRegion(id, bbox, land, water, holes, outDir) {
...
// A CANAL CORRIDOR is carved, not fetched. OSM maps the Panama/Suez canals as a
// patchwork — `natural=water` lake/lock-chamber polygons plus LINEAR `waterway=canal`
// ways for the cut stretches — so `assembleWater` never yields one continuous polygon
// and a trans-isthmus journey has no water thread for A* to route along. A canal region
// is therefore synthesized from its known centerline: one water quad per segment, each
// extended HALF-WIDTH past its joints so consecutive quads OVERLAP — A* only ever sees
// contiguous water, no seam gap at a cell corner. Half-width ≥ ~3 × the A* floor cell
// (0.2 km) so the clearance-inflated search (1 cell) can thread it; channels narrower
// than the real locks/cut get a retry at zero clearance. It stays WIDER than the real
// canal (700 m vs ~200-350 m at the narrows) — acceptable: this path is dashed
// inference, and reachability beats exact channel width.
export function buildCanalRegion({ id, bbox, centerline, halfWidthKm = 0.35, outDir = COAST_DIR }) {
  const lat0 = (bbox.minLat + bbox.maxLat) / 2, lon0 = (bbox.minLon + bbox.maxLon) / 2;
  const kx = 111.32 * Math.cos((lat0 * Math.PI) / 180);   // km per deg lon
  const ky = 111.32;                                      // km per deg lat
  const XY = ([la, lo]) => [(lo - lon0) * kx, (la - lat0) * ky];
  const LL = ([x, y]) => [round(y / ky + lat0), round(x / kx + lon0)];
  const pts = centerline.map(XY);
  ...
  for (let k = 0; k + 1 < pts.length; k++) {
    ...
    const nx = -uy, ny = ux;                 // unit normal (winding irrelevant to pip)
    const hw = halfWidthKm, over = hw * 1.5; // overlap past each joint → contiguous water
    const ax = a[0] - ux * over, ay = a[1] - uy * over;
    const bx = b[0] + ux * over, by = b[1] + uy * over;
    water.push({
      o: [[ax + nx * hw, ay + ny * hw], [bx + nx * hw, by + ny * hw], [bx - nx * hw, by - ny * hw], [ax - nx * hw, ay - ny * hw]].map(LL),
    });
  }
  water.sort((a, b) => b.o.length - a.o.length);
  return emitRegion(id, bbox, [], water, 0, outDir);
}
