...
- Coastline avoidance: `routeAroundLand` in `geo.js` uses a **3-point open-water arc** (entry→apex→exit) around Natural Earth 10m land polygons. Finds the coastline perimeter vertex farthest from the entry→exit chord (apex) and pushes all three points seaward. Each point uses its own LOCAL seaward direction: apex = chord-perpendicular, entry/exit = trail direction (entryPt→a, exitPt→b). This avoids the global-centroid-direction bug where all points on large polygons get collinear pushes, and avoids the edge-normal bug where long polygon edges have midpoints in a different geographic context than the entry/exit point. No coastline-following perimeter points, so Catmull-Rom produces smooth wide arcs without headland wiggles. Synthetic waypoints rendered as dashed `4 4` with reduced opacity. Config in `LAND_AVOIDANCE` (minSegmentKm, dashArray, fadeRatio). Land polygon data lives in `frontend/app/coastline.js` (106 KB, generated by `worker/scripts/build-coastline.mjs`).
...
- `offsetPathSeaward` removed (replaced by `dirFromTo` + `pushPt` in `routeAroundLand`). `simplifyPath` still exported but unused in `routeAroundLand`.
