...
- **Two-layer land test:** `pointOnLand = inLand && !inWater`. A second layer `frontend/app/water.js` (`WATER_POLYGONS`, OSM `natural=water`/`riverbank` per fine zone, `build-water.mjs` + `lib-osm-water.mjs`) re-opens rivers/harbour basins the coastline closes (the upper-Fraser fix). **Resolution policy** (be intentional per destination): simplify tol ≤ ⅓ × narrowest channel to keep open, `routeWater` cellKm ≤ ½ × that; water is always ~25 m. A regional spatial index (so a Singapore boat skips Vancouver polygons) is designed but deferred until disjoint foreign coverage ships. See `worker/CLAUDE.md` → "Water layer" / "Coastline resolution policy".
