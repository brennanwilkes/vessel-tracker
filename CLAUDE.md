...
- Coastline avoidance: `routeAroundLand` in `geo.js` uses a **3-point open-water arc** (entry→apex→exit) around Natural Earth 10m land polygons. Finds the coastline perimeter vertex farthest from the entry→exit chord (apex) and pushes all three points seaward using a **single chord-perpendicular direction** (chord midpoint → apex). All three points use the same direction, which is always perpendicular to the chord on the coastline-bulge side. This avoids the centroid bug (all points pushed same global direction on large polygons), the edge-normal bug (edge midpoint in wrong geographic context), and the trail-direction bug (trail along the chord axis, not perpendicular). No coastline-following perimeter points, so Catmull-Rom produces smooth wide arcs without headland wiggles. Synthetic waypoints rendered as dashed `4 4` with reduced opacity. Config in `LAND_AVOIDANCE` (minSegmentKm, dashArray, fadeRatio). Land polygon data lives in `frontend/app/coastline.js` (106 KB, generated by `worker/scripts/build-coastline.mjs`).
...
- `offsetPathSeaward` removed (replaced by `pushPt` in `routeAroundLand`). `simplifyPath` still exported but unused in `routeAroundLand`.
