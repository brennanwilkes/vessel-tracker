...
- Coastline avoidance: `routeAroundLand` in `geo.js` walks the coastline perimeter around a land polygon, Douglas-Peucker simplifies it (min 5 km tolerance), then pushes ALL simplified vertices seaward in a single direction (chordMid→apex = perpendicular to chord on coastline-bulge side). The perimeter naturally wraps around the coastline, so the arc follows the coastline shape without cutting through land. Uniform push direction avoids the centroid bug (all points pushed same global direction on large polygons), the edge-normal bug (edge midpoint in wrong geographic context), and headland wiggles. Synthetic waypoints rendered as dashed `4 4` with reduced opacity. Config in `LAND_AVOIDANCE` (minSegmentKm, dashArray, fadeRatio). Land polygon data lives in `frontend/app/coastline.js` (106 KB, generated by `worker/scripts/build-coastline.mjs`).
...
- `offsetPathSeaward` removed. `simplifyPath` used in `routeAroundLand` to simplify the coastline perimeter before pushing.
