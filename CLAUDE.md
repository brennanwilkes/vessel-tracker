...
- A* uses a **coast-proximity cost** (`proximityKm`/`proximityWeight` in `routeWater`) so routes bow into open water (wider/natural) and hold channel-centers rather than hugging the coast. `repairOffLand` then nudges any spline bulge off land (chord-clear cases A* can't catch). Geometry is **cached per vessel** (`trailGeom`); on cache miss the trail paints instant straight bridges, then the routed curves fill in **boat-by-boat off the main thread** (`routeQueue`) so first paint stays slick.
